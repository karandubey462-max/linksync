...
linksRouter.get("/", requireAuth, asyncRoute(async (req, res) => {
    const links = await Link.find({ userId: req.userId }).sort({ pinned: -1, order: 1, createdAt: 1 });
    return res.json({ success: true, links });
}));
linksRouter.post("/", requireAuth, asyncRoute(async (req, res) => {
    const parsed = linkSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ success: false, message: parsed.error.issues[0]?.message });
    const urlCheck = await validatePublicUrl(parsed.data.url);
    if (!urlCheck.ok)
        return res.status(urlCheck.statusCode ?? 400).json({ success: false, message: urlCheck.message });
...
linksRouter.put("/:id", requireAuth, asyncRoute(async (req, res) => {
    ...
    const link = await Link.findOneAndUpdate({ _id: req.params.id, userId: req.userId }, {
        ...updateData,
...
linksRouter.delete("/:id", requireAuth, asyncRoute(async (req, res) => {
    if (!mongoose.isValidObjectId(req.params.id))
        return res.status(400).json({ success: false, message: "Invalid link id." });
...
linksRouter.patch("/reset-clicks", requireAuth, asyncRoute(async (req, res) => {
