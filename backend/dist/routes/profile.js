...
profileRouter.get("/", requireAuth, asyncRoute(async (req, res) => {
    const user = await User.findById(req.userId);
    if (!user)
        return res.status(404).json({ success: false, message: "User not found." });
    return res.json({ success: true, user });
}));
profileRouter.put("/", requireAuth, asyncRoute(async (req, res) => {
...
profileRouter.get("/qr", requireAuth, asyncRoute(async (req, res) => {
    const user = await User.findById(req.userId);
    if (!user)
        return res.status(404).json({ success: false, message: "User not found." });
